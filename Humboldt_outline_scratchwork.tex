...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item For this reason, we propose the use of an extended Lennard-Jones (12-10-8-6) potential
	\end{enumerate}
	\item The higher dimensional parameterization of the ex-LJ is computationally infeasible when implementing traditional iterative force field optimization approaches
	\begin{enumerate}
	...
	\end{enumerate}
	\item With this approach, it is possible to ensure that the force field derivative properties are internally consistent with those of the EOS, resulting in better extrapolation for both the EOS and the force field
\end{enumerate}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item Basis functions store the contributions from the $r^{-m}$ terms for the nonbonded energies and forces
	\end{enumerate}
    \item With this approach, it is possible to predict more accurate mixture properties
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
	    \item Reliable estimates of thermophysical properties are extremely important for practical industrial applications
    \end{enumerate}	
    \item Why me?
    \begin{enumerate}
